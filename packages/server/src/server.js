        ...
        try {
            ...
            const officialConfig = {
                ...
                onReady: () => {
                    ...
                    // Send agent-specific greeting to start the conversation
                    try {
                        const greeting = selectedAgent.greeting || 'Hello! Thank you for calling. How can I help you today?';
                        socket.geminiLive.sendText(greeting);
                        console.log(`✅ Successfully sent initial greeting: "${greeting}"`);
                    } catch (error) {
    ...
    async handleTwilioMessage(socket, message) {
        switch (message.event) {
            ...
            case 'start':
                ...
                // NOW setup Gemini client with the correct agent context
            ...
            case 'media':
                ...
                // Check if we have a Gemini client and it's ready
                if (!socket.geminiLive) {
                ...
                if (socket.geminiLive && message.media?.payload) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            // Try to recreate the Gemini client with correct agent
                            const agent = socket.selectedAgent || agentRouter.defaultAgent;
                            this.setupGeminiClientWithAgent(socket, agent).catch(error => {
