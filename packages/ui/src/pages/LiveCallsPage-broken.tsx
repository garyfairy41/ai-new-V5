...
// ActiveCall interface imported from sup                        )}
                      </div>
                    </div>
                  </div>
                ))}
...
export default function LiveCallsPage() {
  ...
  const loadLiveData = async () => {
    ...
    try {
      ...
      // Load call queue (only calls that are actually queued/waiting)
      ...
      // Calculate proper system metrics
      const queuedCallsWaitTimes = actualQueuedCalls.map(call => {
        // Use created_at for pending calls (time since queued) or started_at for in_progress calls
        ...
        if (referenceTime) {
          ...
          const waitTime = Math.floor((now - startTime) / 1000); // wait time in seconds
          // Cap wait time at reasonable maximum (10 minutes = 600 seconds)
  ...
  const setupRealtimeSubscriptions = () => {
    ...
    // Subscribe to call updates (stub implementation - would reload live data)
    const callSubscription = RealtimeService.subscribeToCallUpdates(
      user!.id,
      () => {
        // On update: reload live data
        loadLiveData();
      }
    );

    // Subscribe to agent status updates (stub implementation - would reload agent data)
    const agentSubscription = RealtimeService.subscribeToAgentUpdates(
      ...
      () => {
        // On insert: reload live data
        loadLiveData();
      },
      () => {
        // On delete: reload live data
        loadLiveData();
      }
    );
            ...
            <div className="px-8 py-8">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="mt-2 text-lg text-slate-600">Real-time view of active calls and system status</p>
                </div>
