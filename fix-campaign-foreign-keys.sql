-- Fix foreign key constraints for campaign system

-- Step 1: Drop the existing foreign key constraint from campaign_leads to campaigns
ALTER TABLE campaign_leads 
DROP CONSTRAINT IF EXISTS campaign_leads_campaign_id_fkey;

-- Step 2: Add new foreign key constraint from campaign_leads to outbound_campaigns
ALTER TABLE campaign_leads 
ADD CONSTRAINT campaign_leads_campaign_id_fkey 
FOREIGN KEY (campaign_id) REFERENCES outbound_campaigns(id) ON DELETE CASCADE;

-- Step 3: Verify the constraint was created
SELECT 
    tc.table_name, 
    kcu.column_name, 
    ccu.table_name AS foreign_table_name,
    ccu.column_name AS foreign_column_name,
    tc.constraint_name
FROM 
    information_schema.table_constraints AS tc 
    JOIN information_schema.key_column_usage AS kcu
      ON tc.constraint_name = kcu.constraint_name
      AND tc.table_schema = kcu.table_schema
    JOIN information_schema.constraint_column_usage AS ccu
      ON ccu.constraint_name = tc.constraint_name
      AND ccu.table_schema = tc.table_schema
WHERE tc.constraint_type = 'FOREIGN KEY' 
AND tc.table_name = 'campaign_leads';
